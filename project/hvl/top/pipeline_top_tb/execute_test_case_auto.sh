...
SIM_LOG="$SCRIPT_DIR/sim/simv_auto_run.log"
SIM_BIN="$SCRIPT_DIR/sim/simv_auto"

if [[ ! -x "$SIM_BIN" ]]; then
    echo "FAIL: missing simulator binary: $SIM_BIN"
    echo "Build it first from $SCRIPT_DIR/sim using:"
    echo "  /usr/local/packages/synopsys_2022/vcs/T-2022.06/bin/vcs -full64 -v2005 -debug_all -f ../master_pipeline_top_tb -o simv_auto"
    exit 2
fi
...
(
    cd "$SCRIPT_DIR/sim"
    ./simv_auto > "$SIM_LOG" 2>&1
) || sim_rc=$?

if [[ "$sim_rc" -ne 0 ]]; then
    echo "FAIL: simv_auto exited with rc=$sim_rc"
...
if [[ ! -f "$RESULTS_CMP" ]]; then
    echo "FAIL: results_cmp.txt was not created by simv_auto"
    echo "See simulator log: $SIM_LOG"
