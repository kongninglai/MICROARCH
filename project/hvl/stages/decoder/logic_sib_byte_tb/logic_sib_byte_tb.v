...
module tb_logic_sib_byte();
    ...
    begin
        ...
        $display("  Prefix Count: %0d | SIB_Byte: %h", p_num, sib_byte_true);

        if (expected_flag && (sib_byte_true !== expected_sib)) begin
    ...
    initial begin
        ...
        c[1] = 8'h04; c[2] = 8'h24; 
        verify_scenario("Standard SIB (No Prefix)", 2'd0, 1'b1, 8'h24, 1'b1);

        // --- CASE 2: The "Immediate Trap" ---
        c[1] = 8'h24; 
        verify_scenario("Immediate Trap (is_modrm=0)", 2'd0, 1'b0, 8'h00, 1'b0);

        // --- CASE 3: Register Direct Trap ---
        c[1] = 8'hE0; 
        verify_scenario("Register Direct Trap (Mod=11)", 2'd0, 1'b1, 8'h00, 1'b0);

        // --- CASE 4: Offset SIB (2 Prefixes) ---
        // 66 F2 89 04 24 -> Prefixes@0,1. Opcode@2, ModRM@3, SIB@4.
        c[3] = 8'h04; c[4] = 8'h24; 
        verify_scenario("Offset SIB (2 Prefixes)", 2'd2, 1'b1, 8'h24, 1'b1);

        // --- CASE 5: Max Prefix Lookahead (3 Prefixes) ---
        // Opcode@4, ModRM@5, SIB@6. 
        // This still fits in 6 candidates (candidate6 = index 6).
        c[5] = 8'h04; c[6] = 8'h24; 
        verify_scenario("Max Prefix Lookahead", 3'd3, 1'b1, 8'h04, 1'b1);
