...
module de_to_rr_tb;
    ...
    reg         flush_ex; 
    reg         v_excptn_src_wb, v_ld_cs_src_ex; 
    ...
    wire [2:0]  imm_size;        // FIXED: Changed from [1:0] to [2:0]
    wire        addressing_mode; // FIXED: Changed from [1:0] to 1-bit
    wire [31:0] disp;
    wire [47:0] imm;
    ...
    wire [2:0]  to_rr_imm_size;       // FIXED: Changed from [1:0] to [2:0]
    wire        to_rr_addressing_mode;// FIXED: Changed from [1:0] to 1-bit
    ...
    stage_decode dut_decode (
        .cache_line(cache_line), .o_eip(o_eip_in), .tail_ptr(tail_ptr),
        .cs_limit_reg(cs_limit_reg), .eip_target_ex(eip_target_ex),
        .flush_ex(flush_ex), .v_excptn_src_wb(v_excptn_src_wb),
        .v_ld_cs_src_ex(v_ld_cs_src_ex), .stall_ex(stall_ex),
        .stall_rr(stall_rr), .stall_mem(stall_mem), .stall_wb(stall_wb),
        ...
        .prefix_rep(prefix_rep), .prefix_op_size(prefix_op_size),
        .prefix_seg_ov_id(prefix_seg_ov_id), .prefix_ext(prefix_ext),
        .opcode(opcode), .modrm(modrm), .sib(sib),
        .disp_size_mux(disp_size_mux), .disp(disp), .imm_size(imm_size),
        .imm(imm), .addressing_mode(addressing_mode), .instr_length(instr_length)
    );

    de_to_rr dut_pipe (
        .clk(clk), .rst(rst_bar), // Ensure active-low consistency
        ...
        .from_de_imm_size(imm_size), .from_de_imm(imm),
