module  lshf_bytes_var_256b_tb;//

initial begin
  $vcdplusfile("lshf_bytes_var_256b_tb.dump.vpd");
  $vcdpluson(0, lshf_bytes_var_256b_tb); 
end
...
lshf_bytes_var_256b DUT (
  .in(in), .shf_amt(shf_amt),
  .out(out)
);

lshf_bytes_var_256b_behav REF (
  .in(in), .shf_amt(shf_amt),
  .out(out_exp)
);
