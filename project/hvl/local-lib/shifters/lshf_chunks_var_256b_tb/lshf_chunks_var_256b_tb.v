module  lshf_chunks_var_256b_tb;

initial begin
  $vcdplusfile("lshf_chunks_var_256b_tb.dump.vpd");
  $vcdpluson(0, lshf_chunks_var_256b_tb); 
end
...
lshf_chunks_var_256b DUT (
  .in(in), .shf_amt(shf_amt),
  .out(out)
);

lshf_chunks_var_256b_behav REF (
  .in(in), .shf_amt(shf_amt),
  .out(out_exp)
);
