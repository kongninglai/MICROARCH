module stage_ex #(
    ...
    /*** Function Units ***/
    wire [31:0] regA_rm;
    mux2_32 mux2_regA_rm(regA_rm, f_srcregA, to_ex_load_result[31:0], sig_rm);
    
    // ALU
    wire [31:0] alu_op2;
    wire [31:0] alu_out;
    mux2_32 mux2_alu_op2(alu_op2, f_srcregB, to_ex_imm, sig_alu_srcb_mux);
    ex_alu alu (
        .alu_op         (sig_alu_op),
        .ds             (sig_ds),
        .in0            (regA_rm),
    ...
    wire [7:0] shf_amt;
    wire [31:0] shf_out;
    mux4_8$ mux4_shf_amt(shf_amt, 8'h1, f_srcregC[7:0], to_ex_imm[7:0], 8'bx, sig_shf_srcb_mux[0], sig_shf_srcb_mux[1]);
    ex_shf shf (
        .shf_op(sig_shf_op),
        .ds(sig_ds),
        .shf_data(regA_rm),
    ...
    // PALU
    wire [63:0] MMB_rm;
    mux2_64 mux2_MMB_rm(MMB_rm, f_MMB, to_ex_load_result, sig_rm);
    wire [63:0] palu_out;
    ex_palu palu(
        .dest_in(f_MMA),
        .src_in(MMB_rm),
    ...
    /*** REGFILE WRITE DATA*/
    mux16_32 mux16_gp_wr_data1(from_ex_gp_wr_data_1, alu_out, bsf_out, aaa_out, shf_out, not_out, inc1_out, regA_rm, f_srcregA, 
                                f_srcregB, {16'b0, f_srcSREG}, to_ex_imm, to_ex_load_result[31:0], dec_ecx_out, 32'bx, 32'bx, 32'bx, 
                                sig_gp_dsta_mux[0], sig_gp_dsta_mux[1], sig_gp_dsta_mux[2], sig_gp_dsta_mux[3]);
    
    mux8_32 mux8_gp_wr_data2(from_ex_gp_wr_data_2, inc2_out, to_ex_inc_esp, to_ex_dec_esp, regA_rm, f_srcregB, 32'bx, 32'bx, 32'bx, 
    ...
    mux4_64 mux4_mmx_wr_data(from_ex_mmx_wr_data, palu_out, f_MMB, MMB_rm, 64'bx, sig_mm_dst_mux[0], sig_mm_dst_mux[1]);
