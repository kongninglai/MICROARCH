...
wire          FSM_LD_REGS, FSM_SHF_DATA_WR_MASK, FSM_TAG_WR_MASK_MUX,
              ...
              FSM_IC_MEM_RD_RQ, FSM_ADDR_BUS_ENBAR, 
              FSM_IN_010;
...
wire    [RANK_BURST_SIZE-1:0]               ICC_STREAM_BUF_WR_MASK, ICC_STREAM_BUF_WR_MASK_GATED;

decoder2_4$   decoder2_4$_ICC_STREAM_BUF_WR_MASK(.SEL(counter[1:0]),
                                                 .Y(ICC_STREAM_BUF_WR_MASK), .YBAR());

and2$         and2$_ICC_STREAM_BUF_WR_MASK_GATED[RANK_BURST_SIZE-1:0](ICC_STREAM_BUF_WR_MASK_GATED,
                                                                      ICC_STREAM_BUF_WR_MASK,
                                                                      {RANK_BURST_SIZE{FSM_SB_WR_EN}});

and2$         and2$_FSM_HIT_DATA_MUX(FSM_HIT_DATA_MUX, ICACHE_MISS, ICC_STREAM_BUF_HIT);

assign  FSM_DATA_BUS_SHF_MUX = counter[1:0];

assign  ICC_FSM_VALID_WR_EN_GLOBAL = FSM_TAG_WR_MASK_MUX;
...
wire  [NUM_SETS*NUM_WAYS*RANK_BURST_SIZE-1:0] D0_ICC_DATA_WR_MASK_OUT_01, D1_ICC_DATA_WR_MASK_OUT_01, 
                                              D_ICC_DATA_WR_MASK_OUT_01, D_ICC_DATA_WR_MASK_OUT_11;
wire  [NUM_WAYS-1:0]                          D_ICC_TAG_WR_MASK_OUT_1;
wire  [INDEX_WIDTH+WAY_WIDTH-1:0]             D_ICC_VALID_WR_EN_1;

we_logic_block we_logic_block_inst (           
  .ICACHE_PHYS_ADDR(ICACHE_PHYS_ADDR),
  .ICACHE_VICT_WAY(ICACHE_VICT_WAY),
  .DATA_WR_MASK_OUT(D0_ICC_DATA_WR_MASK_OUT_01),
  .SB_DATA_WR_MASK_OUT(D_ICC_DATA_WR_MASK_OUT_11),
  .TAG_WR_MASK_OUT(D_ICC_TAG_WR_MASK_OUT_1),
  .VALID_WR_EN(D_ICC_VALID_WR_EN_1)
);

wire  [NUM_SETS*NUM_WAYS*RANK_BURST_SIZE-1:0] Q_ICC_DATA_WR_MASK_OUT_01, Q_ICC_DATA_WR_MASK_OUT_10;
wire  [NUM_WAYS-1:0]                          Q_ICC_TAG_WR_MASK_OUT_1;
wire  [INDEX_WIDTH+WAY_WIDTH-1:0]             Q_ICC_VALID_WR_EN_1;

lshf_const #(
  .WIDTH(NUM_SETS*NUM_WAYS*RANK_BURST_SIZE),
  .SHF_AMT(1),
  .SHF_ONES(1)
) lshf_const_D1_ICC_DATA_WR_MASK_OUT_01 (
  .in(Q_ICC_DATA_WR_MASK_OUT_01),
  .out(D1_ICC_DATA_WR_MASK_OUT_01)
);

mux2$   mux2$_D_ICC_DATA_WR_MASK_OUT_01[NUM_SETS*NUM_WAYS*RANK_BURST_SIZE-1:0](
                                                                                D_ICC_DATA_WR_MASK_OUT_01,
                                                                                D0_ICC_DATA_WR_MASK_OUT_01,
                                                                                D1_ICC_DATA_WR_MASK_OUT_01,
                                                                                FSM_SHF_DATA_WR_MASK
                                                                              );

wire    LD_ICC_DATA_WR_MASK_OUT_01;

or2$    or2$_LD_ICC_DATA_WR_MASK_OUT_01(LD_ICC_DATA_WR_MASK_OUT_01,
                                        FSM_SHF_DATA_WR_MASK,
                                        FSM_LD_REGS);
...
reg_n #(
  .WIDTH(NUM_SETS*NUM_WAYS*RANK_BURST_SIZE),
  .USE_EN_BAR(0)
) reg_n_Q_ICC_DATA_WR_MASK_OUT_01 (
  .clk(clk), .rst(rst),
  .en({NUM_SETS*NUM_WAYS*RANK_BURST_SIZE{LD_ICC_DATA_WR_MASK_OUT_01}}), .d(D_ICC_DATA_WR_MASK_OUT_01),
  .q(Q_ICC_DATA_WR_MASK_OUT_01)
);

reg_n #(
  .WIDTH(NUM_SETS*NUM_WAYS*RANK_BURST_SIZE),
  .USE_EN_BAR(0)
) reg_n_Q_ICC_DATA_WR_MASK_OUT_10 (
  .clk(clk), .rst(rst),
  .en({NUM_SETS*NUM_WAYS*RANK_BURST_SIZE{FSM_LD_REGS}}), .d(D_ICC_DATA_WR_MASK_OUT_11),
  .q(Q_ICC_DATA_WR_MASK_OUT_10)
);

reg_n #(
  .WIDTH(NUM_WAYS),
  .USE_EN_BAR(0)
) reg_n_Q_ICC_TAG_WR_MASK_OUT_1 (
  .clk(clk), .rst(rst),
  .en({NUM_WAYS{FSM_LD_REGS}}), .d(D_ICC_TAG_WR_MASK_OUT_1),
  .q(Q_ICC_TAG_WR_MASK_OUT_1)
);

reg_n #(
  .WIDTH(INDEX_WIDTH+WAY_WIDTH),
  .USE_EN_BAR(0)
) reg_n_Q_ICC_VALID_WR_EN_1 (
  .clk(clk), .rst(rst),
  .en({(INDEX_WIDTH+WAY_WIDTH){FSM_LD_REGS}}), .d(D_ICC_VALID_WR_EN_1),
...
reg_n #(
  ...
  .USE_EN_BAR(0)
) reg_n_Q_ICC_ADDR_OUT_1 (
  .clk(clk), .rst(rst),
  .en({(MEM_ADDR_WIDTH-RANK_BURST_SIZE){FSM_LD_REGS}}), .d(ICACHE_PHYS_ADDR),
  .q(Q_ICC_ADDR_OUT_1)
);

assign ICC_TAG_IN = Q_ICC_ADDR_OUT_1[MEM_ADDR_WIDTH-1:MEM_ADDR_WIDTH-1-7];
assign ICC_TAG_VALID_SET_INDEX = Q_ICC_ADDR_OUT_1[RANK_BURST_SIZE+INDEX_WIDTH-1:RANK_BURST_SIZE];

big_increment #(
  .WIDTH(MEM_ADDR_WIDTH-RANK_BURST_SIZE)
) big_increment_D_ICC_NL_PHYS_ADDR (
  .a(ICACHE_PHYS_ADDR),
  .s(D_ICC_NL_PHYS_ADDR)
);

reg_n #(
  .WIDTH(MEM_ADDR_WIDTH-RANK_BURST_SIZE),
  .USE_EN_BAR(0)
) reg_n_Q_ICC_NL_PHYS_ADDR (
  .clk(clk), .rst(rst),
  .en({(MEM_ADDR_WIDTH-RANK_BURST_SIZE){FSM_LD_REGS}}), .d(D_ICC_NL_PHYS_ADDR),
  .q(Q_ICC_NL_PHYS_ADDR)
);
...
stream_buffer stream_buffer_inst (
  .clk(clk), .rst(rst),
  .stream_buffer_wr_mask(ICC_STREAM_BUF_WR_MASK_GATED),
  .icache_addr(ICACHE_PHYS_ADDR), .icache_controller_next_line_addr(Q_ICC_NL_PHYS_ADDR),
  .DATA_BUS_SHF(DATA_BUS_SHF),
  .icache_controller_set_valid(FSM_SET_SB_VALID),
  .stream_buffer_hit(ICC_STREAM_BUF_HIT),
  .stream_buffer_miss(),
...
mux2$   mux2$_ICC_ADDR_OUT[MEM_ADDR_WIDTH-1:RANK_BURST_SIZE](
                                                              ICC_ADDR_OUT,
...
mux2$   mux2$_ICC_WR_DATA_OUT[RANK_BIT_WIDTH-1:0]           (
                                                              ICC_WR_DATA_OUT,
...
mux2$   mux2$_ICC_HIT_DATA_OUT[RANK_BIT_WIDTH-1:0]          (
                                                              ICC_HIT_DATA_OUT,
...
mux4$   mux4$_ICC_DATA_WR_MASK_OUT[NUM_SETS*NUM_WAYS*RANK_BURST_SIZE-1:0]
                                                            (
                                                              ICC_DATA_WR_MASK_OUT,

                                                              ICC_DATA_WR_MASK_DEFAULT,
                                                              Q_ICC_DATA_WR_MASK_OUT_01,
                                                              Q_ICC_DATA_WR_MASK_OUT_10,
                                                              ICC_DATA_WR_MASK_DEFAULT,
...
mux2$   mux2$_ICC_TAG_WR_MASK_OUT[NUM_WAYS-1:0]             (
                                                              ICC_TAG_WR_MASK_OUT,

                                                              {NUM_WAYS{1'b1}},
                                                              Q_ICC_TAG_WR_MASK_OUT_1,
...
tristate_bus_driver1$   tristate_bus_driver1$_ADDR_BUS[MEM_ADDR_WIDTH-1:0]
                                                       (
                                                          ...
                                                          .in({Q_ICC_ADDR_OUT_1, 4'b0000}),
                                                          .out(ADDR_BUS)
                                                       );

tristate_bus_driver1$   tristate_bus_driver1$_IC_MEM_RD_RQ
                                                       (
                                                          .enbar(1'b0),
                                                          .in(FSM_IC_MEM_RD_RQ),
                                                          .out(IC_MEM_RD_RQ)
...
/* Inverters */
        ...
        inv1$ inv_1(CACHE_FILL_DONE_bar, CACHE_FILL_DONE);
        wire Q2_bar;
        wire IC_MEM_RD_ACK_bar;
        inv1$ inv_3(IC_MEM_RD_ACK_bar, IC_MEM_RD_ACK);
        wire SB_FILL_DONE_bar;
        inv1$ inv_4(SB_FILL_DONE_bar, SB_FILL_DONE);
        wire DATA_VALID_BAR_bar;
        inv1$ inv_5(DATA_VALID_BAR_bar, DATA_VALID_BAR);
        wire Q1_bar;
        ...
        and2$ and_0_1(and_0_1_out,ICACHE_MISS,ICC_STREAM_BUF_HIT);
        wire and_1_0_out;
        and4$ and_1_0(and_1_0_out,Q2_bar,Q1,Q0_bar,DATA_VALID_BAR_bar);
        wire and_2_0_out;
        and4$ and_2_0(and_2_0_out,Q2_bar,Q1_bar,Q0_bar,ICACHE_MISS);
        wire and_3_0_out;
        and4$ and_3_0(and_3_0_out,Q2,Q1_bar,Q0,SB_FILL_DONE);
        wire and_4_0_out;
        and4$ and_4_0(and_4_0_out,Q2,Q1_bar,Q0,SB_FILL_DONE_bar);
        wire and_5_0_out;
        and4$ and_5_0(and_5_0_out,Q2_bar,Q1_bar,Q0,IC_MEM_RD_ACK);
        wire and_6_0_out;
        and4$ and_6_0(and_6_0_out,Q2_bar,Q1_bar,Q0,IC_MEM_RD_ACK_bar);
        wire and_7_0_out;
        ...
        or2$ or_12_0(FSM_IC_MEM_RD_RQ,and_5_0_out,and_6_0_out);
        wire or_13_1_out;
        ...
        wire or_14_1_out;
        or4$ or_14_0(ICC_FSM_FILL_BUSY,or_14_1_out,and_5_0_out,and_6_0_out,and_9_0_out);
