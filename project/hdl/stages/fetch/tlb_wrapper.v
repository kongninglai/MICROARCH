...
wire [NUM_TLB_ENTRIES-1:0]  D_WR_TLB_ENTRY_SEL;
...
generate
  for (i = 0; i < NUM_TLB_ENTRIES; i = i + 1) begin : VPN_COMPARE_GEN
    ...
    big_eq #(
      .WIDTH(VPN_BIT_WIDTH)
    ) big_eq_D_WR_TLB_ENTRY_SEL (
      .in0(TLB_ENTRIES_BEHAV[i][TLB_ENTRY_BIT_WIDTH-1:TLB_ENTRY_BIT_WIDTH-VPN_BIT_WIDTH]), 
      .in1(D_WR_TLB_VPN),
      .eq(D_WR_TLB_ENTRY_SEL[i])
    );
...
wire [NUM_TLB_ENTRIES-1:0] D_WR_TLB_ENTRY_SEL_buf16, D_WR_TLB_ENTRY_SEL_buf16_inv;
...
bufferH16$        bufferH16$_D_WR_TLB_ENTRY_SEL_buf16[NUM_TLB_ENTRIES-1:0](D_WR_TLB_ENTRY_SEL_buf16, D_WR_TLB_ENTRY_SEL);
bufferHInv16$     bufferHInv16$_D_WR_TLB_ENTRY_SEL_buf16_inv[NUM_TLB_ENTRIES-1:0](D_WR_TLB_ENTRY_SEL_buf16_inv, D_WR_TLB_ENTRY_SEL);
...
wire [TLB_OUT_BIT_WIDTH-1:0]  D_WR_TLB_OUT_GATED[0:NUM_TLB_ENTRIES-1];

wire [TLB_OUT_BIT_WIDTH-1:0]  ITLB_OUT_COMBINED;
wire [TLB_OUT_BIT_WIDTH-1:0]  D_RD_TLB_OUT_COMBINED;
wire [TLB_OUT_BIT_WIDTH-1:0]  D_WR_TLB_OUT_COMBINED;
...
generate
  for (j = 0; j < NUM_TLB_ENTRIES; j = j + 1) begin : TLB_OUT_GATED_GEN
    ...
    nor2$   nor2$_D_WR_TLB_OUT_GATED[TLB_OUT_BIT_WIDTH-1:0](D_WR_TLB_OUT_GATED[j], TLB_ENTRIES_BEHAV_INV[j][TLB_OUT_BIT_WIDTH-1:0], {TLB_OUT_BIT_WIDTH{D_WR_TLB_ENTRY_SEL_buf16_inv[j]}});
...
generate
  for (k = 0; k < TLB_OUT_BIT_WIDTH; k = k + 1) begin : TLB_OUT_COMBINED_GEN
    ...
    big_or #(
      .WIDTH(NUM_TLB_ENTRIES)
    ) big_or_D_WR_TLB_OUT_COMBINED (
      .out(D_WR_TLB_OUT_COMBINED[k]),
      .in({ D_WR_TLB_OUT_GATED[0][k], D_WR_TLB_OUT_GATED[1][k],
            D_WR_TLB_OUT_GATED[2][k], D_WR_TLB_OUT_GATED[3][k],
            D_WR_TLB_OUT_GATED[4][k], D_WR_TLB_OUT_GATED[5][k],
            D_WR_TLB_OUT_GATED[6][k], D_WR_TLB_OUT_GATED[7][k]})
    );
...
assign D_WR_TLB_PFN_OUT            = D_WR_TLB_OUT_COMBINED[TLB_OUT_BIT_WIDTH-1:TLB_OUT_BIT_WIDTH-PFN_BIT_WIDTH];
assign D_WR_TLB_WRITE_DISABLE_OUT  = D_WR_TLB_OUT_COMBINED[PAGE_LEVEL_WRITE_DISABLE_BIT_POS];
assign D_WR_TLB_CACHE_ENABLE_OUT   = D_WR_TLB_OUT_COMBINED[PAGE_LEVEL_CACHE_ENABLE_BIT_POS];
...
nand3$  nand3$_D_WR_TLB_PAGE_FAULT_OUT(D_WR_TLB_PAGE_FAULT_OUT, D_WR_TLB_HIT, D_WR_TLB_OUT_COMBINED[VALID_BIT_POS], D_WR_TLB_OUT_COMBINED[PRESENT_BIT_POS]);
